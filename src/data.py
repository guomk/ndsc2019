...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        im_path = os.path.join(self.root_dir, self.data.iloc[idx % self.data.shape[0]]['image_path'])
        if im_path[-4:] != '.jpg':
            im_path = im_path + '.jpg'
        image = Image.open(im_path)
        if image.mode != 'RGB':
            return self.__getitem__(np.random.randint(self.data.shape[0]))

        data_sample = {
            'image': image,
            'label': self.data.iloc[idx]['category_label'],
...
def get_train_time_transform(scale_size, crop_size):
